...
describe("POST /logout", () => {
    ...
    it("returns 200 for valid logout", async () =>{
        ...
        const logoutUser = await supertest(app).post("/logout")
        .set("authorization", `Bearer ${token}`).send();
        expect(logoutUser.status).toEqual(200);
    });
    it("returns 401 for unauthorized requests", async () => {
        ...
        const logoutUserBad = await supertest(app).post("/logout")
        .set("authorization", null).send();
        expect(logoutUserBad.status).toEqual(401);
    });
    beforeEach(async () => {
